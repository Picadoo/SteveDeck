...
| 项 | 原因 | 收尾命令 |
|---|---|---|
| **引擎 Docker 镜像构建** | 本机未装 Docker | `docker compose -f docker/docker-compose.yml up -d --build`（Dockerfile/compose 已就绪并 review） |
| **安卓 APK 构建（Phase 5）** | 本机未装 Android SDK/NDK（数 GB，属重型安装，按用户偏好未擅自安装） | 见 `docs/BUILD.md#安卓客户端`；UI 已响应式适配并验证 |
| **真实「机器人上线」集成测试** | 无可用 MC 测试服 | 引擎走的是已在原项目生产运行的复用代码；可用 flying-squid 本地服做集成 |

## 后续增强（设计内、非阻塞）📋

- 脚本系统：✅ 已实现并验证（新建/保存/运行/停止 + 触发器 + JSON 编辑器，引擎落盘 `scripts.json`）。仅「积木拖拽」可视化编辑器留作后续（当前为 JSON 编辑器）。
- 背包网格 UI、垃圾清理/NPC 交互卡片：引擎已支持，UI 待补。
