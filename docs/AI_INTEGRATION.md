...
- **动作**：`chat(msg)` `cmd(cmd)` `whisper(player,msg)` `goto(x,y,z)` `goto_location(name)`（自动执行该地点配置的前置指令/到达脚本，跨世界传送用它） `goto_nearest(target,distance)` `return_home`（回名为「家」的保存地点，没有则用追怪返回点） `look_at(target)` `hold(key,s)` `sneak(active)` `equip(item)`（按**显示名或物品ID**找并持到手） `equip_best_weapon` `equip_best_tool(block)` `drop(item,count)` `drop_all(keep)` `deposit(item, location?)`（可选 location=保存地点名：先去仓库地点再就近存箱） `use_item`（右键手持物，常用于打开菜单） `interact(target)`（右键实体/NPC） `attack(entity,count,interval)` `dig(block,distance)`（挖最近的该方块） `place(item,x,y,z)`（在坐标放方块） `craft(item,count)`（自动找工作台） `jump` `swap_hands` `look(x,y,z)`
...
[
  ...
  {
    "name": "run_script",
    "description": "在机器人上保存并运行一个脚本（脚本格式见 SteveDeck 文档）",
    "parameters": {
      "type": "object",
      "properties": {
        "botId": { "type": "string" },
